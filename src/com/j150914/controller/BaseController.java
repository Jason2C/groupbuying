package com.j150914.controller;
...
public class BaseController extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 得到请求参数 如/SzMVC/userAct.do
		String act = request.getRequestURI();
		// 截取 /userAct.do 最后/和截取.do 截取后 userAct
		...
		try {
			if (act.indexOf("~") == -1) {
				...
				String[] act_mth = act.split("~");
				// 得到calssPath
				String classPath = properties.getProperty(act_mth[0]);
				// 找到Actionclass
				Class actClass = Class.forName(classPath);
				// 得到一个对象
				IAction action = (IAction) actClass.newInstance();
		...
		while (pmNames.hasMoreElements()) {
			...
			try {
				if (fieldName.indexOf(".") == -1) {
					...
					// 取得需要封装的pojo
					Object pojo = actClass.getMethod(field2Method(2, mtd_field[0])).invoke(action);
					// System.out.println(pojo.getClass());
					Method fieldSetter = pojoClz.getMethod(field2Method(1, mtd_field[1]), fieldClz);
					// 封装值
